...
def main(args):
    ...
    if args['figure2']:
    	...
    	if torch.cuda.is_available():
    		...
    		device = 'cpu'
    	debye_calc = DebyeCalculator(device=device, verbose=1, qmin=1, qmax=25, qstep=0.01, biso=0.3)
    ...
    if args['figure3']:
    	...
    	def dummy_calculations():
    	    ...
    	    # You can use any computation-heavy tasks here
    	    # For example, matrix multiplications or element-wise operations
    	...
    	debye_calc_cpu = DebyeCalculator(device='cpu', verbose=1, qmin=1, qmax=25, qstep=0.1, biso=0.3)
    	for i in trange(len(radii_cpu)):
    	    timings_DC_CPU = []
    	...
    	if torch.cuda.is_available():
    	    ...
    	    # Now your GPU should be warmed up and ready for profiling
    	    print("GPU warmed up.")
    	    ...
    	    for i in trange(len(radii_cuda)):
    	        timings_DC_GPU = []
    	        for _ in range(repetitions):
    	...
    	mu_CMI, sigma_CMI = [], []
    	for i in trange(len(radii_diffpy)):      
    	    timings_CMI = []
    	    for _ in range(repetitions):
    	        with tempfile.TemporaryDirectory() as tmpdirname:
...
if __name__ == '__main__':	
    ...
    parser.add_argument('--cpu_radius_max', type=float, default=13)
    parser.add_argument('--cuda_radius_max', type=float, default=20)
    parser.add_argument('--diffpy_radius_max', type=float, default=20)
    parser.add_argument('--figure2', type=bool, default=False)
    parser.add_argument('--figure3', type=bool, default=False)
