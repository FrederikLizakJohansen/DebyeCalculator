...
def test_iq():
    ...
    ph = np.loadtxt('unittests_files/icsd_001504_cc_r6_lc_2.85_6_tetragonal_Iq.dat')
    Q_expected, Iq_expected = ph[:,0], ph[:,1]

    # Calculate the scattering intensity using the DebyeCalculator
    calc = DebyeCalculator()
    Q, Iq = calc.iq('unittests_files/icsd_001504_cc_r6_lc_2.85_6_tetragonal.xyz')

    # Check that the calculated scattering intensity matches the expected value
    assert np.allclose(Q, Q_expected), f"Expected Q to be {Q_expected}, but got {Q}"
    assert np.allclose(Iq, Iq_expected), f"Expected Iq to be {Iq_expected}, but got {Iq}"

def test_sq():
    # Load the expected structure factor from a file
    ph = np.loadtxt('unittests_files/icsd_001504_cc_r6_lc_2.85_6_tetragonal_Sq.dat')
    Q_expected, sq_expected = ph[:,0], ph[:,1]

    # Calculate the structure factor using the DebyeCalculator
    calc = DebyeCalculator()
    Q, sq = calc.sq('unittests_files/icsd_001504_cc_r6_lc_2.85_6_tetragonal.xyz')

    # Check that the calculated structure factor matches the expected value
    assert np.allclose(Q, Q_expected), f"Expected Q to be {Q_expected}, but got {Q}"
    assert np.allclose(sq, sq_expected), f"Expected Sq to be {sq_expected}, but got {sq}"

def test_fq():
    # Load the expected atomic form factor from a file
    ph = np.loadtxt('unittests_files/icsd_001504_cc_r6_lc_2.85_6_tetragonal_Fq.dat')
    Q_expected, fq_expected = ph[:,0], ph[:,1]

    # Calculate the atomic form factor using the DebyeCalculator
    calc = DebyeCalculator()
    Q, fq = calc.fq('unittests_files/icsd_001504_cc_r6_lc_2.85_6_tetragonal.xyz')

    # Check that the calculated atomic form factor matches the expected value
    assert np.allclose(Q, Q_expected), f"Expected Q to be {Q_expected}, but got {Q}"
    assert np.allclose(fq, fq_expected), f"Expected fq to be {fq_expected}, but got {fq}"

def test_gr():
    # Load the expected radial distribution function from a file
    ph = np.loadtxt('unittests_files/icsd_001504_cc_r6_lc_2.85_6_tetragonal_Gr.dat')
    r_expected, gr_expected = ph[:,0], ph[:,1]

    # Calculate the radial distribution function using the DebyeCalculator
    calc = DebyeCalculator()
    r, gr = calc.gr('unittests_files/icsd_001504_cc_r6_lc_2.85_6_tetragonal.xyz')

    # Check that the calculated radial distribution function matches the expected value
    assert np.allclose(r, r_expected), f"Expected r to be {r_expected}, but got {r}"
    assert np.allclose(gr, gr_expected), f"Expected Gr to be {gr_expected}, but got {gr}"

def test_get_all():
    # Calculate Iq, Fq, Sq, and Gr using the DebyeCalculator
    calc = DebyeCalculator()
    r, q, iq, sq, fq, gr = calc._get_all('unittests_files/icsd_001504_cc_r6_lc_2.85_6_tetragonal.xyz')
    ...
    ph = np.loadtxt('unittests_files/icsd_001504_cc_r6_lc_2.85_6_tetragonal_Gr.dat')
    r_expected, gr_expected = ph[:,0], ph[:,1]
    assert np.allclose(gr, gr_expected), f"Expected r to be {gr_expected}, but got {gr}"
    assert np.allclose(gr, gr_expected), f"Expected Gr to be {gr_expected}, but got {gr}"
...
def test_invalid_input():
    ...
    with pytest.raises(ValueError):
        ...
        calc.iq('unittests_files/icsd_001504_cc_r6_lc_2.85_6_tetragonal.xyz')